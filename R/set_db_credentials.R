...
set_db_credentials <- function(username, password, install = TRUE){
  ...
  if (install) {
    ...
    if (!file.exists(renv)){
      ...
      if(any(grepl("DB_USER_AD", tv))){
        ...
        if (ans == 1){
          ...
          newenv <- oldenv[!grepl("DB_USER_AD|DB_PWD_AD", oldenv)]
          ...
          stop(cat(crayon::red(cli::symbol$cross), "Your DB_USER_AD was not updated."), call. = FALSE)
    ...
    userconcat <- sprintf("DB_USER_AD='%s'", username)
    pwdconcat <- sprintf("DB_PWD_AD='%s'", password)

    # Append API key to .Renviron file
    write(userconcat, renv, sep = "\n", append = TRUE)
    write(pwdconcat, renv, sep = "\n", append = TRUE)

    cat(crayon::green(cli::symbol$tick), 'Your username and password have been stored in your .Renviron and can be accessed by Sys.getenv("DB_USER_AD") and Sys.getenv("DB_PWD_AD"). \nTo use now, restart R or run `readRenviron("~/.Renviron")`')
    ...
    cat(crayon::green(cli::symbol$tick), "DB_USER_AD and DB_PWD_AD set for current session. To install your usenrame and password for use in future sessions, run this function with `install = TRUE`.")
  }
}
